...
const SwiperCategory = ({ userData }) => {
  ...
  const [foodItems4, setFoodItems4] = useState([]);
  const [foodItems5, setFoodItems5] = useState([]);
      ...
      db.collection("food")
        ...
        .onSnapshot((snapshot) => {
          const foodData = snapshot.docs
            ...
            .filter((food) => food.daySchedule[today])
  ...
  useEffect(() => {
    if (selectedCategory) {
      ...
      db.collection("food")
        .where("foodCategory", "==", selectedCategory.categoryName)
        .where("timeschedule", 'array-contains', 'Breakfast')
        .onSnapshot((snapshot) => {
          const foodData3 = snapshot.docs
            .map((doc) => {
              return {
                id: doc.id,
                ...doc.data(),
  ...
  useEffect(() => {
    if (selectedCategory) {
      ...
      db.collection("food")
        .where("foodCategory", "==", selectedCategory.categoryName)
        .where("timeschedule", 'array-contains', 'Lunch')
        .onSnapshot((snapshot) => {
          const foodData4 = snapshot.docs
            .map((doc) => {
              return {
                id: doc.id,
                ...doc.data(),
  ...
  useEffect(() => {
    if (selectedCategory) {
      ...
      db.collection("food")
        .where("foodCategory", "==", selectedCategory.categoryName)
        .where("timeschedule", 'array-contains', 'Dinner')
        .onSnapshot((snapshot) => {
          const foodData5 = snapshot.docs
            .map((doc) => {
              return {
                id: doc.id,
                ...doc.data(),
  ...
  const handleFavouriteItem = async (foodName, increment = true) => {
    try {
      const usersRef = db.collection("users");
      const querySnapshot = await usersRef
        .where("name", "==", userData.name)
        ...
        if (increment) {
          ...
          if (!foodQuerySnapshot.empty) {
            ...
            await foodDoc.ref.update({
              favouriteRating: favouriteRating - 1,
            });
