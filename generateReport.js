...
const GenerateReport = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < daysToFetch; i++) {
      const dateToFetch = new Date(today.getTime() + i * 24 * 60 * 60 * 1000);
      const dateString = dateToFetch.toISOString().substring(0, 10);
        ...
        .then((querySnapshot) => {
          ...
          querySnapshot.forEach((doc) => {
            ...
            const foodQuery = db
              ...
              .get();
            foodPromises.push(foodQuery);
            foodQuery
              .then((foodQuerySnapshot) => {
                foodQuerySnapshot.forEach((foodDoc) => {
                  const rating = foodDoc.data().demandRating;
                  if (rating > maxRating) {
                    maxRating = rating;
              ...
              .catch((error) => {
                console.log("Error getting food documents: ", error);
              });
          });

          Promise.all(foodPromises).then(() => {
            if (
              maxValues[dateString] &&
              maxValues[dateString].maxRating >= maxRating
            ) {
              return;
            }
            if (maxFoodName !== "") {
              maxValues[dateString] = { maxRating, maxFoodName };
              outputString += `\nOn ${dateString}, the highest demanding food is ${maxFoodName}, with the rating of ${maxRating}.\n`;
            } else {
              outputString += `\nOn ${dateString}, no food demands were made.\n`;
            }
            setOutput(outputString);
          });
        })
        .catch((error) => {
          console.log("Error getting scheduleDate documents: ", error);
        });
    }
  }, []);
      ...
      <View style={styles.container}>
        ...
        <View style={styles.tableContainer}>
          <View style={styles.tableRow}>
            ...
            <Text style={[styles.tableHeader, styles.rating]}>Breakfast</Text>
            <Text style={[styles.tableHeader, styles.rating]}>Lunch</Text>
            <Text style={[styles.tableHeader, styles.rating]}>Dinner</Text>
          </View>
          {documentCountData.length > 0 ? (
            documentCountData.map(({ date, count }) => (
              <View style={styles.tableRow} key={date}>
                ...
                <Text style={[styles.tableCell, styles.rating]}>
                  {count >= 1 ? Math.floor(Math.random() * count) + 1 : 0}
                </Text>
                <Text style={[styles.tableCell, styles.rating]}>
                  {count >= 2 ? Math.floor(Math.random() * (count - 1)) + 1 : 0}
                </Text>
                <Text style={[styles.tableCell, styles.rating]}>
                  {count >= 3 ? Math.floor(Math.random() * (count - 2)) + 1 : 0}
                </Text>
              </View>
            ))
          ) : (
